...
const EarnSection = ({ currentLanguage }) => {
    const navigate = useNavigate();

    return (
        <section className="bg-slate-50 w-full h-[40rem] relative earn-section flex_col_center p-12">
            <div className="mx-auto w-full h-[50%] relative flex_col_center pr-2">
                <div className="text-center flex_col_center">
                    <h2 className="mb-10 text-3xl font-bold leading-tight">
                        {currentLanguage === "en" ? "Earn with Rickshaw" : "রিক্শার মাধ্যমে আয় করুন"}
                    </h2>
                    <p className="mb-6">
                        {currentLanguage === "en" ? "Join the highest earning platform in the country" : "জয়েন করুন দেশের সর্বোচ্চ আয়ের প্ল্যাটফর্মে"}
                    </p>
                    <div className="w-44 flex_center h-16 bg-blue-500 hover:bg-blue-400 rounded-md shadow-lg text-center text-white cursor-pointer" onClick={() => {
                        ...
                        {currentLanguage === "en" ? "Start Earning" : "আয় শুরু করুন"}
